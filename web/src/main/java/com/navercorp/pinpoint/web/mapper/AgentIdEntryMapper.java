...
public class AgentIdEntryMapper implements RowMapper<List<AgentIdEntry>> {
    ...
    public List<AgentIdEntry> mapRow(Result result, int rowNum) throws Exception {
        ...
        // parse status cell
        Cell statusCell = result.getColumnLatestCell(HbaseTables.AGENT_ID.getName(), HbaseTables.AGENT_ID_STATUS_QUALIFIER);
        long statusTimestamp = 0;
        AgentLifeCycleState agentState = AgentLifeCycleState.UNKNOWN;
        if (statusCell != null) {
            Buffer statusBuffer = new OffsetFixedBuffer(statusCell.getValueArray(), statusCell.getValueOffset(), statusCell.getValueLength());
            statusTimestamp = statusBuffer.readLong();
